...
<head>
	...
	<script type="text/javascript">
		...
		<s:if test="!listAccesLocal.isEmpty()">
			$('#textNoLogin').hide();
		</s:if>
		showDomaineLogin();
		}
...
<body onload="onLoad();">
	<s:include value="/jsp/user_pages/commun/html_body_top.jsp">
		<s:param name="titre">
			<s:property value="#session.TITLE_IN_SESSION.get('MAC_ADD')" />
		</s:param>
	</s:include>

	<div class="contentTable">
		<s:form theme="simple" id="mainForm" action="createMachineListAction" method="POST">
			...
			<table class="formTable" width="800px">
				...
				<col width="35%" />
				<tr>
					<td align="left">Nom <span class="starObligatoire">*</span> :
					</td>
					<td align="left"><s:textfield id="nom" name="nom"
				...
				<tr>
					<td align="left">Type <span class="starObligatoire">*</span> :
					</td>
					<td align="left"><s:select id="type" name="type"
							list="%{listMachineType}" listKey="id" listValue="type"
							headerKey="" headerValue="" data-validation-engine="validate[required]" /></td>
					<td align="left">Site <span class="starObligatoire">*</span> :
					</td>
					<td align="left"><s:select id="site" name="site"
							list="%{listMachineSite}" listKey="id" listValue="site"
							headerKey="" headerValue="" data-validation-engine="validate[required]" /></td>
				</tr>
				<tr>
					<td align="left">OS <span class="starObligatoire">*</span> :</td>
					<td align="left"><s:select id="os" name="os"
							list="%{listMachineOS}" listKey="id" listValue="Nom_OS"
							headerKey="" headerValue="" data-validation-engine="validate[required]" /></td>
					<td align="left">Service gérant :</td>
					<td align="left"><s:select id="interlocuteur"
							name="interlocuteur" list="%{listMachineInterlocuteur}"
							listKey="id" listValue="nomService" headerKey="-1" headerValue="" /></td>
				</tr>
				<tr>
					<td align="left">Domaine <span class="starObligatoire">*</span>
						:</td>
					<td align="left"><s:select id="domaine" name="domaine"
							list="%{listMachineDomaine}" listKey="id"
							listValue="nomDomaine" headerKey="" headerValue=""
							onchange="showDomaineLogin()"
							data-validation-engine="validate[required]" /></td>
					<td align="left">Environnement :</td>
					<td align="left"><s:select id="environnement"
							name="environnement" list="%{listMachineEnvironnement}"
							listKey="id" listValue="environnement" headerKey="-1"
							headerValue="" /></td>
				</tr>
				<tr>
					<td align="left">Commentaire :</td>
					<td align="left"><textarea
							id="commentaire<s:property value="#stat.index" />"
							name="commentaire<s:property value="#stat.index" />"
							style='width: 97%' rows='3'>
			...
			<div class="plus">
				Ajouter :
				<s:select id="appToAdd" name="appToAdd" listKey="id"
					listValue="applicatif" headerKey="-1" headerValue=""
					list="%{listApplicatif}" />
				<s:a href="#" onclick="javascript:addApplication()">
					<img class="icone" alt="Ajouter" title="Ajouter"
				...
				<tbody>
					...
					<s:else>
						<s:iterator value="listApplications" status="stat">
							<tr>
								<td align="left">&nbsp;&nbsp;<s:property value="appName" />
								</td>
								<td align="center"><a href="#"
									onclick="javascript:deleteApplication(<s:property value="appID"/>);">
										<img class="icone" alt="Supprimer" title="Supprimer"
										src="<s:property value="#session.ENSEIGNE" />/img/supprimer-16.png" />
								</a> <input type="hidden"
									id="application<s:property value="#stat.index" />"
									name="application<s:property value="#stat.index" />"
									value="<s:property value="appID"/>" />
								</td>
							</tr>
						</s:iterator>
					</s:else>
			...
			<div class="contentTableLeft">
				...
				<div class="plus">
					<s:a href="#" onclick="javascript:addIp()">
						<img class="icone" alt="Ajouter" title="Ajouter"
							src="<s:property value="#session.ENSEIGNE" />/img/ajouter-16.png" />
					</s:a>
				</div>
				...
				<table class="dataTable" rules="all" id="tableIp">
					...
					<tbody>
						...
						<s:iterator value="listIPs" status="stat">
							<tr>
								<td align="center"
									id="IPNumber<s:property value="#stat.index" />"><input
									type="text" id="ip<s:property value="#stat.index" />"
									name="ip<s:property value="#stat.index" />"
									value="<s:property value="IP"/>" />
								</td>
								<td align="center"
									id="IPComment<s:property value="#stat.index" />"><textarea
										id="commentaire<s:property value="#stat.index" />"
										name="commentaire<s:property value="#stat.index" />"
										onkeypress="return ( this.value.length < 100 );"
										style='width: 80%' rows='3'>
										<s:property value="Commentaire" />
									</textarea>
								</td>
								<td align="center"><a href="#"
									onclick="javascript:deleteIp(<s:property value="#stat.index" />);">
										<img class="icone" alt="Supprimer" title="Supprimer"
										src="<s:property value="#session.ENSEIGNE" />/img/supprimer-16.png" />
								</a>
								</td>
			...
			<div class="contentTableRight">
				<div class="titreTable">
					<b>Acc&egrave;s local à la machine</b>
				</div>
				<div class="plus">
						...
						<s:iterator value="listAccesLocal" status="stat">
							<tr id="listAccesLocal">
								<td align="center"><input type="text"
									name="login<s:property value="#stat.index" />"
									id="login<s:property value="#stat.index" />"
									value="<s:property value="login"/>" style="width: 90%" /></td>
								<td align="center"><input type="text"
									name="password<s:property value="#stat.index" />"
									id="password<s:property value="#stat.index" />"
									value="<s:property value="password"/>" style="width: 90%" /></td>
								<td align="center"><select
									id='domaine<s:property value="#stat.index" />'
									name='domaine<s:property value="#stat.index" />'>
										<option value='-1'
											<s:if test="domaine == '-1'">SELECTED</s:if>></option>
										<s:iterator value="listMachineDomaine">
											<option value='<s:property value='id'/>'
												<s:if test="domaine == id">SELECTED</s:if>>
								...
								<td align="center"><a href="#"
									onclick="javascript:deleteLogin(<s:property value="#stat.index" />);">
										<img class="icone" alt="Supprimer" title="Supprimer"
										src="<s:property value="#session.ENSEIGNE" />/img/supprimer-16.png" />
								</a>
								</td>
