	...
	<script type="text/javascript">
		function valider() {
			...
			if (mainForm.nomMeteo.value.replace(/^\s+/g, '').replace(/\s+$/g, '') == "") {
				alert("Veuillez entrer le nom de la Météo");
...
<body>
	<s:include value="/jsp/user_pages/commun/html_body_top.jsp">
		<s:param name="titre"><s:property value="#session.TITLE_IN_SESSION.get('MTG_MET_M')" /></s:param>
	</s:include>

	<div class="contentTable" style="width: 45%">
		<s:form id="mainForm" theme="simple" action="modifyMeteoAction" method="POST">
			<s:token></s:token>
			<s:hidden name="meteoID" id="meteoID" value="%{meteo.id}"/>
			<s:hidden name="deleteService" id="deleteService" value="-1"/>
			
			<table class="formTable">
				<tr>
					<td align="left">Nom de la Météo <span class="starObligatoire">*</span> :</td>
					<td align="left"><s:textfield name="nomMeteo" maxlength="100" size="35" value="%{meteo.nom_meteo}"/></td>
				</tr>			
			</table>
