...
public class ExportExcel {
	...
	private static final String PERIODE = "P�riode";
	private static final String COLLABORATEUR = "Collaborateur";
	private static final String TELEPHONE = "T�l�phone";
	...
	public static HSSFWorkbook exportCRA(String nomIntervenant,
		...
		/******* 3 PREMIERS ENCADRES *******/
		int i;
		row = sheet.createRow(rowIndex); // cr�ation de la ligne 2
		...
		rowIndex = rowIndex + 2;
		row = sheet.createRow(rowIndex); // cr�ation de la ligne 11
		...
		row = sheet.createRow(++rowIndex); // cr�ation de la ligne 12
		...
		for (rowIndex = 20; rowIndex < 22; ++rowIndex) {
			row = sheet.createRow(rowIndex); // cr�ation de la ligne 20 et 21
			for (int j = 3; j < 6; j++) {
				cell = row.createCell(j);
				cell.setCellStyle(centerStyleTitleColumn);
				if (j == 3)
			...
			for (int j = 8; j < 13; j++) {
				...
				cell.setCellStyle(centerStyleTitleColumn);
				if (j == 8)
					cell.setCellValue("Rècapitulatif des \n Absences Hebdomadaires");
			}
		...
		row = sheet.createRow(rowIndex); // cr�ation de la ligne 22
		...
		for (rowIndex = 23; rowIndex < 30; ++rowIndex) {
			row = sheet.createRow(rowIndex); // cr�ation de la ligne 23
			cell = row.createCell(2);
			cell.setCellStyle(centerStyleBold);
			if (rowIndex == 23)
				cell.setCellValue("1ère");
		...
		row = sheet2.createRow(2); // cr�ation de la ligne 2
		for (i = 3; i < 11; i++) {
			cell = row.createCell(i);
			cell.setCellStyle(centerStyleTitleColumn);
			if (i == 3)
		...
		for (int l = 4; l < 9; l++) {
			...
			for (int c = 1; c < 6; c++) {
				...
				if (l == 5 && c == 1) // MOIS
					cell.setCellValue("Socièté Cliente :");
				if (l == 5 && c == 3) // MOIS
					cell.setCellValue("ITCE");
				if (l == 6 && c == 1) // MOIS
					cell.setCellValue("Projet :");
		...
		row = sheet2.createRow(11); // cr�ation de la ligne 11
		...
		row = sheet2.createRow(12); // cr�ation de la ligne 12
		cell = row.createCell(1);
		...
		row = sheet2.createRow(13); // cr�ation de la ligne 13
		...
		row = sheet2.createRow(21); // cr�ation de la ligne 21
	...
	public static HSSFWorkbook exportPlanningSemaine(
		...
		for (Planning_Vacation planning_vacation : listeCouleur) {
			...
			// on reconstitue le code couleur r�cup�r�e si il ne fait pas partie
			// de la palette
		...
		for (Map<String, String> ligneUser : mapSemaine) {
			...
			if (!couleur.equals("")) {
				...
				// on reconstitue le code couleur r�cup�r�e si il ne fait pas
	...
	public static HSSFWorkbook exportPlanningMoisPilote(
		...
		// Definition des style de carat�res
		// Style du font
		...
		// D�finition des style de cellules
		...
		// reconstitution de la date selectionn�e
		...
		try {
			...
			for (Users u : listeUser) {
				...
				for (int i = 1; i <= nbDayOfTheMonth; i++) {
					...
					if (!Couleur.equals("")) {
						// on reconstitue le code couleur r�cup�r�e si il ne
	...
	// methode � compl�ter pour l'export planning mois
	public static HSSFWorkbook exportPlanningMois(Date dateSelected,
		...
		// Titre du tableau
		// on a besoin de la date s�lectionn�e ainsi que l'equipe s�lectionn�e
		// pour compl�ter le titre
		...
		// semaine � la date s�lectionn�e et pour l'equipe s�lectionn�e
		...
		try {
			...
			for (int i = 0; i < 42; i++) {
				...
				if (!Couleur.equals("")) {
					// on reconstitue le code couleur r�cup�r�e si il ne fait
