...
public class CreateFicheIncidentsQualiteAction extends AbstractAction{

	private static final long serialVersionUID = -813366993926394668L;
	...
	protected String executeMetier() {
		try{				
			...
			IncidentsQualiteDatabaseService.create(dateE, description, cause_raison, incidence, echeance, statut, dateR);
			
			HistoriqueDatabaseService.create(null, MessageFormat.format(getText("historique.incidents.qualite.creation"), new Object[]{IncidentsQualiteDatabaseService.getId(dateE, description)}), (Users)session.get(PilotageConstants.USER_LOGGED), PilotageConstants.HISTORIQUE_MODULE_INCIDENTS);
			info = getText("incidents.qualite.creation.valide");

			return OK;
		}
		catch(Exception e){
			error = getText("error.message.generique") + " : " + e.getMessage();
			erreurLogger.error("Creation d'une fiche incident qualite", e);
