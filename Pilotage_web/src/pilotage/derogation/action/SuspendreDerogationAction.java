...
public class SuspendreDerogationAction extends AbstractAction{

	private static final long serialVersionUID = -5815695275980139373L;
	...
	protected String executeMetier() {
		try {		
			DerogationDatabaseService.changeStatut(selectRow, PilotageConstants.DEROGATION_ETAT_SUSPENDUE);
			...
			//envoi du mail
			MailService.sendEmail(emails, PilotageConstants.MAILSERVER_DEROGATION_SUSPENDRE_SUBJECT, PilotageConstants.MAILSERVER_DEROGATION_SUSPENDRE_TEXT, content, null);
			
			info = MessageFormat.format(getText("derogation.valideur.suspendue"), new Object[]{selectRow});
			HistoriqueDatabaseService.create(null, MessageFormat.format(getText("historique.derogation.valideur.suspendre"), new Object[]{selectRow}), (Users)session.get(PilotageConstants.USER_LOGGED), PilotageConstants.HISTORIQUE_MODULE_DEROGATION);
			return OK;
		}
		catch (Exception e) {
			error = getText("error.message.generique") + " : " + e.getMessage();
			erreurLogger.error("Suspension d'une dérogation - ", e);
			return ERROR;
		}
	}
}
