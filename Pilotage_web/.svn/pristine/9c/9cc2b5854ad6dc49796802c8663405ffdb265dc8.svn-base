...
<head>
	...
	function ajouter() {
		submitData("redirectCreateAdminDestinataireNoc.action");
	}
	...
	function supprimer(id){
		if(confirm("Etes vous sur de vouloir supprimer le destinataire NOC ?")){
			changeData('selectedID', id);
			submitData('DeleteDestinataireNoc.action');
		}
	}
...
<body>
	<s:include value="/jsp/user_pages/commun/html_body_top.jsp">
		<s:param name="titre"><s:property value="#session.TITLE_IN_SESSION.get('DNOC_LST')" /></s:param>
	</s:include>
	<div class="contentTable" >
		<s:form id="mainForm" theme="simple" action="showAdminDestinataireNoc">
			<s:token></s:token>
			<s:hidden name="page" id="page" />
			<s:hidden name="nrPerPage" id="nrPerPage"/>
			<s:hidden name="nrPages" value="%{pagination.nrPages}" />
			<s:hidden name="selectedID" id="selectedID"/>
			<s:if test="#session.USER_DROITS.contains('DEST_NOC_ADD')">
		...
		<table class="dataTable" rules="all">
				...
				<s:if test="destDebNoc.isEmpty()">
					...
					<s:else>
						<s:iterator value="destDebNoc" status="status">
							<tr>
								...
								<td align="center">
									...
									<s:if test="#session.USER_DROITS.contains('DEST_NOC_DEL')">
