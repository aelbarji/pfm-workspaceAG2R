...
<head>
	...
	function valider() {
		...
		function retour(){
			submitData('showDerogationTypeChangementAction.action');
		}
	</script>
</head>
<body>
	<s:include value="/jsp/user_pages/commun/html_body_top.jsp">
		<s:param name="titre"><s:property value="#session.TITLE_IN_SESSION.get('DRG_TCH_M')" /></s:param>
	</s:include>
	
	<div class="contentTable" style="width: 40%">
		<s:form theme="simple" id="mainForm" action="modifyDerogationTypeChangementAction">
			<s:hidden name="id"/>
			<s:hidden name="libelleChanged" value="false"/>
			<s:token></s:token>
			<table class="formTable">
				<tr>
					<td align="left" colspan="2">Veuillez entrer le libellé du type de changement</td>
				</tr>
				<tr>
					<td>Libellé <span class="champObligatoire">*</span> : </td>
					<td><s:textfield name="libelle" id="libelle" maxlength="30" size="60" data-validation-engine="validate[required]"></s:textfield></td>
				</tr>
				<tr>
