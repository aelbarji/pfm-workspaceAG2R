...
public class ValiderDerogationAction extends AbstractAction{
	...
	protected String executeMetier() {
		try {		
			DerogationDatabaseService.changeStatut(selectRow, PilotageConstants.DEROGATION_ETAT_VALIDEE);
			...
			//envoi du mail
			MailService.sendEmail(emails, PilotageConstants.MAILSERVER_DEROGATION_VALIDER_SUBJECT, PilotageConstants.MAILSERVER_DEROGATION_VALIDER_TEXT, content, null);
			
			info = MessageFormat.format(getText("derogation.valideur.validee"), new Object[]{selectRow});
			HistoriqueDatabaseService.create(null, MessageFormat.format(getText("historique.derogation.valideur.validation"), new Object[]{selectRow}), (Users)session.get(PilotageConstants.USER_LOGGED), PilotageConstants.HISTORIQUE_MODULE_DEROGATION);
			return OK;
		}
		catch (Exception e) {
			error = getText("error.message.generique") + " : " + e.getMessage();
			erreurLogger.error("Validation d'une dérogation - ", e);
			return ERROR;
		}
	}
}
