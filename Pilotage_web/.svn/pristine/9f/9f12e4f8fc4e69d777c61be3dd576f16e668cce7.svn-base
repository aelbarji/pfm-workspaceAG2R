...
<head>
	...
	function ajouter() {
		submitData("redirectCreateComService.action");
	}

	function modify(id){		
		changeData('selectedID', id);
		submitData('redirectModifyComService.action');	
	}

	function supprimer(id){
		if(confirm("Etes vous sur de vouloir supprimer le service ?")){
			changeData('selectedID', id);
			submitData('deleteComService.action');
		}
	}
...
<body>
	<s:include value="/jsp/user_pages/commun/html_body_top.jsp">
		<s:param name="titre"><s:property value="#session.TITLE_IN_SESSION.get('SER_COM_LST')" /></s:param>
	</s:include>
	<div class="contentTable" >
		<s:form id="mainForm" theme="simple" action="showComService">
			<s:token></s:token>
			...
			<s:if test="#session.USER_DROITS.contains('COM_SER_ADD')">
		...
		<table class="dataTable" rules="all">
				...
				<thead>
					...
					<tr>
						<th class="titreColonne">Nom Service</th>
						<th class="titreColonne">Actions</th>
					</tr>
				</thead>
				<tbody>
				<s:if test="listComServ.isEmpty()">
						<tr>
