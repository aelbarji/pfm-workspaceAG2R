...
function valider() {
	...
	function retour() {
		submitData('showEnvironnementTypeAction.action');
	}
</script>
</head>
<body>
	<s:include value="/jsp/user_pages/commun/html_body_top.jsp">
		<s:param name="titre">
			<s:property value="#session.TITLE_IN_SESSION.get('ENV_TYP_M')" />
		</s:param>
	</s:include>

	<div class="contentTable" style="width: 40%">
		<s:form theme="simple" id="mainForm"
			action="modifyEnvironnementTypeAction" method="POST">
			<s:token></s:token>
			<s:hidden name="selectRow" />
			<s:hidden name="principalChanged" value="false" />
			<s:hidden name="libelleChanged" value="false" />
			<table class="formTable">
				<tr>
					<td align="left" colspan="2">Veuillez entrer le libellé du
						type d'environnement</td>
				</tr>
				<tr>
					<td>Libellé <span class="champObligatoire">*</span> :</td>
					<td><s:textfield name="libelle" id="libelle" maxlength="25"
							size="44" onkeydown="submitOnKeyPress(event);" data-validation-engine="validate[required]"></s:textfield>
					</td>
				</tr>
				<tr>
